...
export function useP2PHost(hostInfo: HostInfo): P2PHostPhase {
  ...
  const [myPlayerId] = useState(() => nanoid(10));
  ...
  const gameStateRef = useRef<GameState | null>(null); // authoritative — mutated only via applyAction
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        handle = await connectAsHost(generateRoomCode(), {
        ...
        playersRef.current.set(myPlayerId, { playerId: myPlayerId, name: hostInfo.name, color: hostInfo.color, isHost: true });
        syncLobbyState();
        setPhase('lobby');
      } catch (err) {
